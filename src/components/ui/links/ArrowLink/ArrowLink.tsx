...
export const ArrowLink = function ArrowLink<C extends React.ElementType>({
  children,
  className,
  direction = 'right',
  as,
  ...
  const Component = as || TextLink;

  return (
    <Component {...rest} className={clsx('group gap-[0.25em]', direction === 'left' && 'flex-row-reverse', className)}>
      <span>{children}</span>
      <svg
        viewBox="0 0 16 16"
        height="1em"
